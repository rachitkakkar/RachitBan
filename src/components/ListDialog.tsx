...
function ListDialog({ dialog, addList, closeDialog } : ListDialogProps) {
            ...
            <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
              <div>
                <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                  <h3 className="text-base font-semibold leading-6 text-gray-900" id="Dialog-title">Add List</h3>
                  <form onSubmit={(e) => inputSubmit(e) } className="bg-white rounded pt-2">
                    <input value={name} onChange={(e) => setName(e.target.value)} className="appearance-none w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" placeholder="New List"/>
                  </form>
                </div>
              </div>
            </div>
